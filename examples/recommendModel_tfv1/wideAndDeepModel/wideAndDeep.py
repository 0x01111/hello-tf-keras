...
class wideAndDeep(object):
    ...
    def input_output(self):
        ...
        self.batch_fem = tf.placeholder(tf.float32, [None, self.feature_length, self.embedding_length],
                                        name="batch_fem")
        # keep_prob
    ...
    def forward_propagation(self):
        ...
        wide和deep
        """
        wfd = tf.get_variable(name="wfd", shape=[self.feature_length + self.hidden_layer[len(self.hidden_layer)-1], 1], dtype=tf.float32,
                                  initializer=tf.truncated_normal_initializer(mean=0, stddev=0.1))
        bfd = tf.get_variable(name="bfd", shape=[1], dtype=tf.float32,
                              initializer=tf.truncated_normal_initializer(mean=0, stddev=0.01))
        y_concat = tf.concat([self.wide, y_deep], 1)    # None * (n+隐藏层最后一层节点数量)
...
if __name__ == "__main__":
    ...
    # 搭建wideAndDeep网络
    model = wideAndDeep(0.005, feature_length, 5, 100, [80, 80, 80])
    model.build_graph()
