...
class deepFMKaggle(object):
    ...
    def forward_propagation(self):
        ...
        self.interaction = 0.5 * tf.subtract(
            ...
            tf.matmul(tf.pow(self.x, 2), tf.pow(self.embedding, 2)))  # None*k
        ...
        FM和deep
        """
        wfd = tf.get_variable(name="wfd", shape=[self.feature_length + self.embedding_length + self.hidden_layer[len(self.hidden_layer)-1], 1], dtype=tf.float32,
                                  initializer=tf.truncated_normal_initializer(mean=0, stddev=0.1))
        bfd = tf.get_variable(name="bfd", shape=[1], dtype=tf.float32,
                              initializer=tf.truncated_normal_initializer(mean=0, stddev=0.01))
        y_concat = tf.concat([self.linear_dropout, self.interaction_dropout, y_deep], 1)    # None * (n+k+隐藏层最后一层节点数量)
        self.y_predict = tf.sigmoid(tf.add(tf.matmul(y_concat, wfd), bfd))
...
if __name__ == "__main__":
    ...
    # 搭建deepFMKaggle网络
    model = deepFMKaggle(0.005, feature_length, 5, 100, [80, 80, 80])
    model.build_graph()
